...
function teach(robot, varargin)
    ...
    if isempty(c)
        % doesnt exist in current axes, look wider
    ...
    %   [l b w h]
    set(ax, 'OuterPosition', [0.30 0 0.70 1])
    ...
    for i=1:n
        ...
        uicontrol(panel, 'Style', 'text', ...
            'Units', 'normalized', ...
            'BackgroundColor', bgcol, ...
            'Position', [0 height*(n-i+2) 0.15 height], ...
            'String', sprintf('q%d', i));
        
        % slider itself
        q(i) = max( qlim(i,1), min( qlim(i,2), q(i) ) ); % clip to range
        handles.slider(i) = uicontrol(panel, 'Style', 'slider', ...
            'Units', 'normalized', ...
            'Position', [0.15 height*(n-i+2) 0.65 height], ...
            'Min', qlim(i,1), ...
            'Max', qlim(i,2), ...
            'Value', q(i), ...
        ...
        handles.edit(i) = uicontrol(panel, 'Style', 'edit', ...
            'Units', 'normalized', ...
            'Position', [0.80 height*(n-i+2)+.01 0.20 height], ...
            'BackgroundColor', bgcol, ...
            'String', num2str(qscale(i)*q(i), 3), ...
    ...
    for i=1:n
        ...
        set(handles.edit(i), ...
            'Interruptible', 'off', ...
            'Callback', @(src,event)teach_callback(src, robot.name, i, handles));
